...
- [x] 9.3 Verify HITL flow: agent calls ask_human → workflow pauses → SendHumanInput signal → workflow resumes
- [x] 9.4 Verify cancel flow: CancelAgentRun → workflow cancelled → pod cleaned up
- [x] 9.5 Verify TTL: create AgentRun with short TTL → workflow times out → pod cleaned up
