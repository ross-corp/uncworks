...
- [x] 3.2 Build Go-based RPC Gateway Sidecar and verify with gRPC contract tests.
- [x] 3.3 Create a base Docker image with `devbox` and `bun` runtime pre-installed.
- [x] 3.4 Implement automated tests for the `devbox shell` execution context.

## 4. Agent Harness (pi-mono Extension)

- [x] 4.1 Develop the `pi-aot-extension` (TypeScript) and verify with Bun test runner.
- [x] 4.2 Implement OTel tracing and verify span emission via an OTel Collector sidecar.
- [x] 4.3 Implement `/ask_human` tool and verify HITL signaling via a mock gRPC client.
