...
func TestStore_Queue(t *testing.T) {
	...
	if err := store.Enqueue(ctx, &QueueEntry{
		ID: "q-1", AgentRunID: "ar-low", Priority: 0,
		UserID: "user1", CreatedAt: now,
	}); err != nil {
		t.Fatalf("Enqueue: %v", err)
	}
	if err := store.Enqueue(ctx, &QueueEntry{
		ID: "q-2", AgentRunID: "ar-high", Priority: 10,
		UserID: "user2", CreatedAt: now,
	}); err != nil {
		t.Fatalf("Enqueue: %v", err)
	}
	...
	// Dequeue empty queue returns nil
	entry, err = store.Dequeue(ctx)
	if err != nil {
